...
import { Plus, Pencil, Trash2, Loader2, GraduationCap, X, Save, Calendar, MapPin } from "lucide-react";
...
const empty: Partial<Education> = { degree: "", fieldOfStudy: "", institution: "", location: "", startDate: "", isCurrent: false, displayOrder: 0 };

export default function AdminEducationPage() {
  const { data: items, loading, refetch } = useData<Education[]>("/education");
  ...
  const save = async () => {
    setSaving(true);
    try {
      if (modal.data.id) await api.put(`/education/${modal.data.id}`, modal.data);
      else await api.post("/education", modal.data);
  ...
  const del = async (id: string) => {
    ...
    try { await api.delete(`/education/${id}`); toast.success("Deleted!"); refetch(); } catch { toast.error("Failed."); }
  };

  const set = (key: keyof Education, value: any) => setModal(m => ({ ...m, data: { ...m.data, [key]: value } }));

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2"><GraduationCap size={22} /> Education</h1>
          <p className="text-slate-400 text-sm mt-1">Manage your academic background</p>
        </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-slate-500 text-xs mt-1">
                    ...
                    {item.cgpa && <span>CGPA: {item.cgpa}/{item.maxCgpa ?? "4.00"}</span>}
                  </div>
                </div>
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="glass rounded-2xl border border-white/10 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="flex items-center justify-between p-5 border-b border-white/5">
                <h3 className="text-white font-semibold">{modal.data.id ? "Edit" : "Add"} Education</h3>
                <button onClick={() => setModal({ open: false, data: empty })} className="text-slate-400 hover:text-white"><X size={18} /></button>
              </div>
              <div className="p-5 grid sm:grid-cols-2 gap-4">
                {([["Degree", "degree"], ["Field of Study", "fieldOfStudy"], ["Institution", "institution"], ["Location", "location"]] as [string, keyof Education][]).map(([label, field]) => (
                  <div key={field}>
                ...
                <div>
                  <label className="text-slate-400 text-xs mb-1 block">CGPA</label>
                  <input type="number" step="0.01" value={modal.data.cgpa ?? ""} onChange={e => set("cgpa", e.target.value ? Number(e.target.value) : undefined)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500/60 transition-all" />
                </div>
                <div>
                  <label className="text-slate-400 text-xs mb-1 block">Max CGPA</label>
                  <input value={modal.data.maxCgpa ?? "4.00"} onChange={e => set("maxCgpa", e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500/60 transition-all" />
                </div>
                <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="current-edu" checked={modal.data.isCurrent ?? false} onChange={e => set("isCurrent", e.target.checked)} className="w-4 h-4 accent-indigo-500" />
                  <label htmlFor="current-edu" className="text-slate-300 text-sm">Currently studying here</label>
                </div>
                <div className="sm:col-span-2">
                  <label className="text-slate-400 text-xs mb-1 block">Description</label>
                  <textarea value={modal.data.description ?? ""} onChange={e => set("description", e.target.value)} rows={3} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500/60 transition-all resize-none" />
                </div>
              </div>
